...
class MinQuaternion:
    ...
    def qmul(self,q1,q2):
        ...
        x1, y1, z1, w1 = np.split(q2, 4, axis=-1)16260
    ...
    def qdiv(self,q1,q2):
        quaternionObj1 = Quaternion(q1.a,q1.b,q1.c,q1.d)
        temp = q2.qinv()
        quaternionObj2 = Quaternion(temp.a,temp.b,temp.c,temp.d)
        Q1_div_Q2 = quaternionObj1.mul(quaternionObj2)
        #dont forget to cast to MinQuaternion
        return Q1_div_Q2
    
    #5
    def qinv(self):
        abs2here =(self.a)**2+(self.b)**2+(self.c)**2+(self.d)**2
        #Q1_inv = Quaternion((self.a) - (self.b) - (self.c) -(self.d) ) /( (self.a)**2+(self.b)**2+(self.c)**2+(self.d)**2)
        Q1_inv = MinQuaternion(self.a/abs2here, -self.b/abs2here, -self.c/abs2here, -self.d/abs2here)
        return Q1_inv
    
        
    #6
    def qabs(self,q1):
        x = sqrt( ( (self.a)**2+(self.b)**2+(self.c)**2+(self.d)**2))
        return x
